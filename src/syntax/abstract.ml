...
let rec desugar_expression = function
  ...
  | ELambda ([], _) ->
      ...
      binder")
  | ELambda ([x], e1) -> Lambda (desugar_binder x, desugar_expression e1)
  | ELambda (x::xs, e1) ->
      Lambda (desugar_binder x, desugar_expression (ELambda (xs, e1)))
  | EPi (x, e1, e2) ->
      Pi (desugar_binder x, desugar_expression e1, desugar_expression e2)
  | ESigma (x, e1, e2) -> 
      Sigma (desugar_binder x, desugar_expression e1, desugar_expression e2)
...
and desugar_declaration = function
  | DLet (Ident x, ps, e1, e2) ->
      let rec handle_parameters t e = function
        | [] -> (t, e)
        | (Param (b, e1))::ps ->
            handle_parameters (Pi (desugar_binder b, desugar_expression e1, t))
            (Lambda (desugar_binder b, e)) ps in
      let t, e =
...
and desugar_parameter = function
  | Param (x, e) -> (desugar_binder x, desugar_expression e)
...
let rec resugar_expression = function
  ...
  | Lambda (b, e) ->
      let rec collect_binders l = function
        | Lambda (b, e) -> collect_binders (b::l) e
        | e -> (l, e) in
      let bs, e = collect_binders [b] e in
      ELambda (rev (map resugar_binder bs), resugar_expression e)
  | Pi (Underscore, e1, e2) -> 
      EArrow (resugar_expression e1, resugar_expression e2)
  | Pi (b, e1, e2) ->
      EPi (resugar_binder b, resugar_expression e1, resugar_expression e2)
  | Sigma (Underscore, e1, e2) ->
      ETimes (resugar_expression e1, resugar_expression e2)
  | Sigma (b, e1, e2) -> 
      ESigma (resugar_binder b, resugar_expression e1, resugar_expression e2)
...
and resugar_declaration = function
  | Let (x, e1, e) ->
      (match resugar_expression e with
       | ELambda (bs, e) ->
           let rec collect_binders ps t = function
             | [] -> (ps, t, [])
             | b::bs ->
                 (match t with
                  | Pi (b2, e1, e2) ->
                      let b2, e1 = resugar_binder b2, resugar_expression e1 in
                      if b = b2 then
                        collect_binders ((Param (b, e1))::ps) e2 bs
                      else (ps, t, b::bs)
                  | _ -> (ps, t, b::bs)) in
           (match collect_binders [] e1 bs with 
  ...
  | Type (x, ps, e, cs) ->
      DType (Ident x
      , map (fun (b, e) -> Param (resugar_binder b, resugar_expression e)) ps
