...
(** type checks a single pattern
    ...
    pattern does not match typ, or Some (pattern, matched_value, i', context', env',
    subst), where
     * matched_value is a value which is matched by the patterns, with neutral
       variables for all binders
     * i' is the level to be used in checking the body of the clause
     * context' is the context with all of the binders in the pattern added env' is the
...
val add_binders :
  ...
  -> pattern
  -> (pattern * value * int * Equality.constraints * Context.t * value Environment.t
