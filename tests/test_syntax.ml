...
let test_parser = "parser" >::: (map test_eq_parser [
  ...
  ReplDeclaration ([DLet (Ident "f", EArrow (EIdentifier (Ident "Nat"), EArrow
  (ETimes (EIdentifier (Ident "Nat"), EIdentifier (Ident "Bool")), EIdentifier
  (Ident "Nat"))), ELambda ([BName (Ident "x"); BUnderscore], EApplication
  (EIdentifier (Ident "succ"), EIdentifier (Ident "x"))))], SEMISEMI ";;"));
...
let test_desugar = "desugar" >::: (map test_eq_desugar [
  ...
  "A"), Identifier "zero")) ])]);
  ("t5", ReplExpression (ELambda ([BName (Ident "x"); BName (Ident "y")], EUnit),
